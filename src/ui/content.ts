...
export const KNOB_META: KnobMeta[] = [
  ...
  },
  {
    ...
    step: 10,
    unit: 'IOPS',
    hint: 'Aggregate host commands per second offered to the device. Everything downstream scales from here.',
  },
  {
    key: 'requestSizeKiB',
    label: 'Request size',
    ...
    step: 4,
    unit: 'KiB',
    hint: 'Mean transfer size. Large sequential requests amortise the per-command hops; small random ones multiply them.',
  },
  {
    key: 'randomShare',
    label: 'Random share',
  ...
  },
  {
    ...
    step: 1,
    unit: 'entries',
    hint: 'Max entries the controller fetches from ONE submission queue at a time. Small caps per-flow throughput — the fairness lever of FAST 2018 §6.1.4.',
  },
  {
    key: 'dataCacheMiB',
    label: 'Data cache',
    guc: 'Data_Cache_Capacity',
    group: 'memory',
    kind: 'logrange',
    min: 16,
    max: 4096,
    step: 16,
    unit: 'MiB',
    hint: 'Device-side DRAM write cache. A deep-queue writer thrashes it: evicting before destaging completes pushes extra flash traffic onto the other flow.',
  },
  {
    key: 'cmtCapacityMiB',
    label: 'CMT capacity',
    ...
    step: 1,
    unit: 'MiB',
    hint: 'Cached mapping table in DRAM. A CMT miss stalls the request behind a mapping read from NAND — often longer than the read itself.',
  },
  {
    key: 'cacheSharing',
    label: 'Cache sharing',
    guc: 'Data_Cache_Sharing_Mode',
    group: 'memory',
    kind: 'select',
    options: [
      { value: 'SHARED', label: 'shared — one cache, all flows compete' },
      { value: 'EQUAL_PARTITIONING', label: 'equal partitioning — one cache per flow' },
    ],
    hint: 'How the DRAM cache is divided among concurrent flows. Partitioning isolates a thrashing writer; sharing lets it evict everyone.',
  },
  {
    key: 'readCacheMode',
    label: 'Cache mode',
    guc: 'Device_Level_Data_Caching_Mode',
    group: 'memory',
    kind: 'select',
    options: [
      { value: 'WRITE_CACHE', label: 'WRITE_CACHE — absorb writes only' },
      { value: 'READ_CACHE', label: 'READ_CACHE — cache reads only' },
      { value: 'WRITE_READ_CACHE', label: 'WRITE_READ_CACHE — both' },
      { value: 'TURNED_OFF', label: 'TURNED_OFF — every access reaches flash' },
    ],
    hint: 'What the device cache caches. READ_CACHE helps read-mostly flows; WRITE_CACHE is the default absorbing path.',
  },
  {
    key: 'overprovisioning',
    label: 'Overprovisioning',
    guc: 'Overprovisioning_Ratio',
    group: 'checkpoint',
    kind: 'range',
    min: 0.04,
    max: 0.28,
    step: 0.01,
    unit: 'reserved',
    hint: 'Reserved space behind the logical capacity. More spare area means GC reclaims cheaper blocks less often.',
  },
  {
    key: 'gcExecThreshold',
    label: 'GC threshold',
    guc: 'GC_Exect_Threshold',
    group: 'checkpoint',
    kind: 'range',
    min: 0.02,
    max: 0.3,
    step: 0.01,
    unit: 'free',
    hint: 'GC starts when the free-page pool drops below this share. Raising it cleans early and cheap; lowering it defers the storm.',
  },
  {
    key: 'preemptibleGc',
    label: 'Preemptible GC',
    guc: 'Preemptible_GC_Enabled',
    group: 'checkpoint',
    kind: 'toggle',
    hint: 'GC suspends an erase when a user read is waiting. Off means GC owns the die for the whole erase.',
  },
  {
    key: 'gcHardThreshold',
    label: 'GC hard threshold',
    guc: 'GC_Hard_Threshold',
    group: 'checkpoint',
    kind: 'range',
    min: 0.01,
    max: 0.05,
    step: 0.005,
    unit: 'free',
    hint: 'Below this free share, even preemptible GC runs without pausing — the device is nearly out of room.',
  },
  {
    key: 'schedulingPolicy',
    label: 'Scheduling policy',
    guc: 'Transaction_Scheduling_Policy',
    group: 'sim',
    kind: 'select',
    options: [
      { value: 'OUT_OF_ORDER', label: 'OUT_OF_ORDER — Sprinkler-style' },
      { value: 'PRIORITY_OUT_OF_ORDER', label: 'PRIORITY_OUT_OF_ORDER — honours NVMe classes' },
    ],
    hint: 'How the FTL scheduler orders transactions across dies.',
  },
]
